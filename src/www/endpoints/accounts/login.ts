...
export const accountsLoginRoutes = (app: ElysiaApp) =>
  app.use(verifyCSRF()).post("/login/", async ({ jwt, cookie: { scratchsessionid }, body: { username, password } }) => {
    ...
    response: t.Tuple([t.Object({
      ...
      success: t.Union([t.Literal(0), t.Literal(1)], { description: "ログインに成功したか(0: 失敗, 1: 成功)" }),
